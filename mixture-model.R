...
## Gaussian mixture model for PER and salary clustering 


library(mclust)
wnba_mclust <- salary_stat |> 
     select(Salary, per) |> 
     Mclust()

wnba_mclust |> 
     augment() |> 
     ggplot(aes(x = per, y = Salary, color = .class, size = .uncertainty)) +
     geom_point(alpha = 0.6) +
     ggthemes::scale_color_colorblind()

# how do clusters compare to positions?


table("Clusters" = nba_mclust$classification, "Protection Status" = salary_stat$`Protection Status`)
