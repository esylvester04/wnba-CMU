...
ggplot(salary_stat, aes(x = per, y = salary_clean)) +
  # Highlight rookie salary range with transparent red rectangle
  annotate("rect",
           xmin = -Inf, xmax = Inf,
           ymin = 66000, ymax = 78000,
           fill = "red", alpha = 0.15) +
  ...
  # Labels for key players with repulsion to avoid overlap
  geom_label_repel(data = highlight_all,
                   aes(label = player),
                   size = 3,
                   box.padding = 0.3,
  ...
  geom_hline(yintercept = 80000, linetype = "dashed", color = "red", alpha = 0.5) +
  geom_hline(yintercept = 208000, linetype = "dashed", color = "green") +
  geom_hline(yintercept = 245000, linetype = "dashed", color = "blue") +
  
  # Labels off to the right with no box background, just text with white halo
  geom_text(aes(x = 45, y = 73000, label = "Rookie Salary Range\n($66k–$78k)"),
            size = 3, hjust = 0,
            color = "red",
            stroke = 0.2,  # creates a thin white outline
            fontface = "bold") +
  geom_text(aes(x = 45, y = 208000, label = "Regular Max Salary\n($208k)"),
            size = 3, hjust = 0,
            color = "green",
            stroke = 0.2,
            fontface = "bold") +
  geom_text(aes(x = 45, y = 242000, label = "Supermax Salary\n($242k)"),
            size = 3, hjust = 0,
            color = "blue",
            stroke = 0.2,
            fontface = "bold") +
  ...
  coord_cartesian(xlim = c(0, 55)) +  # more space on right
  theme_classic(base_size = 14) 
