...
ggplot(salary_stat, aes(x = per, y = Salary)) +
  ...
  labs(
    title = "Player Efficientcy Rating vs. WNBA Salary (2025 WNBA Season)",
    x = "Player Efficiency Rating (PER)",
    y = "Salary (USD)"
  ) +
  scale_y_continuous(labels = dollar) +
  scale_x_continuous(breaks = seq(0, 35, by = 5)) +  # set clean breakpoints
  theme_minimal()
...
# Optional: highlight top 5 salary players with labels
top_salary_players <- salary_stat %>%
  arrange(desc(Salary)) %>%
  slice_head(n = 5)

ggplot(salary_stat, aes(x = per, y = Salary)) +
  geom_point(alpha = 0.6, color = "#1f78b4", size = 3) +  # nicer transparency & size
 # geom_smooth(method = "lm", se = TRUE, color = "darkred", linetype = "dashed") +  # regression line
  geom_text_repel(
    data = top_salary_players,
    aes(label = player),
    size = 3.5,
    box.padding = 0.4,
    point.padding = 0.3,
  ...
  labs(
   ...
   # subtitle = "With linear regression trendline and top 5 highest salary players labeled",
    x = "Player Efficiency Rating (PER)",
    y = "Salary (USD)"
  ) +
